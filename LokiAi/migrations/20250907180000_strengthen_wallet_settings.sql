...
-- This migration enhances the existing table for better performance and data integrity
...
-- Add partial indexes for boolean flags (removed CURRENT_TIMESTAMP as it's not immutable)
CREATE INDEX IF NOT EXISTS idx_wallet_settings_active_users ON wallet_settings(wallet_address, last_accessed);
...
BEGIN
    ...
    WHERE last_accessed < CURRENT_TIMESTAMP - (days_inactive || ' days')::INTERVAL
    AND created_at < CURRENT_TIMESTAMP - (days_inactive || ' days')::INTERVAL;
