...
export async function authenticateKeystroke(requestData: {
  username: string
  password?: string
  features?: number[]
  holdTimes?: number[]
  ...
  try {
    console.log("Authentication request received for:", requestData.username)
    ...
    // Try to load model from localStorage (demo) or IndexedDB
    const modelKey = `ghost_key_model_${username}`
    const modelDataStr = localStorage.getItem(modelKey)
    ...
    } else if (requestData.extractedFeatures && Array.isArray(requestData.extractedFeatures)) {
      features = requestData.extractedFeatures
    } else {
      // Build features from components
      const holdTimes = requestData.holdTimes || []
      const ddTimes = requestData.ddTimes || []
      const udTimes = requestData.udTimes || []
      const typingSpeed = requestData.typingSpeed || 0
      const flightTime = requestData.flightTime || 0
      const errorRate = requestData.errorRate || 0
      const pressPressure = requestData.pressPressure || 0
    ...
    if (modelData.modelType === "autoencoder" && modelData.autoencoder) {
      ...
      const logKey = `ghost_key_auth_log`
      ...
      const existingLogs = JSON.parse(localStorage.getItem(logKey) || '[]')
      existingLogs.push(logEntry)
      localStorage.setItem(logKey, JSON.stringify(existingLogs.slice(-100)))
