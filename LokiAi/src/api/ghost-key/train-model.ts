...
export async function trainKeystrokeModel(requestData: {
  username: string
  features: number[]
  holdTimes?: number[]
  ddTimes?: number[]
  ...
  try {
    const { username, features, holdTimes, ddTimes, udTimes, additionalFeatures, sampleCount, privacyMode, rawData } = requestData

    // Store sample data in localStorage (demo mode)
    const sampleKey = `ghost_key_sample_${username}_${sampleCount}`
    const sampleData = {
    ...
    // Store raw data if not in privacy mode
    if (!privacyMode && rawData) {
      const rawDataKey = `ghost_key_raw_${username}_${sampleCount}`
      localStorage.setItem(rawDataKey, JSON.stringify({
        sampleId: sampleCount,
        timestamp: new Date().toISOString(),
    ...
    if (sampleCount >= AUTH_CONFIG.SAMPLES_REQUIRED - 1) {
      ...
      for (let i = 0; i <= sampleCount; i++) {
        try {
          const sampleKey = `ghost_key_sample_${username}_${i}`
          const sampleDataStr = localStorage.getItem(sampleKey)
          if (sampleDataStr) {
